...
export const ReportPrintModal: React.FC<ReportPrintModalProps> = ({
  ...
  // Editable report signers with persistence
  const [signerConfig, setSignerConfig] = useState<ReportSignerConfig>(() => {
    try {
      const saved = localStorage.getItem('addarasa_report_signers');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (parsed && typeof parsed === 'object') {
          return {
            creatorName: parsed.creatorName || currentUser?.name || 'Administrator',
            creatorTitle: parsed.creatorTitle || currentUser?.role || 'Staff Inventaris & Keuangan',
            approverName: parsed.approverName || 'H. Ahmad Fauzi (Owner / Pemilik)',
            approverTitle: parsed.approverTitle || 'Owner / Direktur Utama',
            signatureCity: parsed.signatureCity || 'Tangerang Selatan',
          };
        }
      }
    } catch {
    ...
    return {
      ...
      creatorTitle: currentUser?.role || 'Staff Inventaris & Keuangan',
      approverName: 'H. Ahmad Fauzi (Owner / Pemilik)',
      approverTitle: 'Owner / Direktur Utama',
      signatureCity: 'Tangerang Selatan',
    };
  });

  const [isEditingSigners, setIsEditingSigners] = useState<boolean>(false);

  useEffect(() => {
    try {
      localStorage.setItem('addarasa_report_signers', JSON.stringify(signerConfig));
    } catch {
      // ignore
    }
  }, [signerConfig]);
  ...
  const handlePrint = () => {
    const docTitle = getDocTitle();
    setPrintStatusMessage('Menyiapkan dan membuka dialog cetak printer...');
    printElement(printableRef.current, docTitle, (msg) => {
      ...
      <div className="bg-white rounded-2xl max-w-5xl w-full max-h-[94vh] flex flex-col shadow-2xl border border-[#c4c5d5]/40 overflow-hidden">
        ...
        <div className="p-4 md:p-5 border-b border-[#c4c5d5]/30 bg-[#fbf8ff] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Edit Signer Names Button */}
            <button
              type="button"
              onClick={() => setIsEditingSigners((prev) => !prev)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-bold transition-all cursor-pointer border ${
                isEditingSigners
                  ...
                  : 'bg-[#f4f2fc] text-[#00288e] border-[#c4c5d5]/50 hover:bg-[#e8e7f1]'
              }`}
              title="Ubah Nama Admin & Owner pada template tanda tangan laporan"
            >
              <span className="material-symbols-outlined text-[16px]">draw</span>
              <span>{isEditingSigners ? 'Tutup Pengaturan Nama' : 'Edit Nama Admin & Owner'}</span>
            </button>
        ...
        {/* Expandable Signer Configuration Panel */}
        {isEditingSigners && (
          <div className="bg-[#eeedf7] px-4 py-3.5 border-b border-[#c4c5d5]/60 animate-in slide-in-from-top-2 duration-150">
            <div className="max-w-[840px] mx-auto">
              <div className="flex items-center justify-between mb-2.5">
                <div className="flex items-center gap-2">
                  <span className="material-symbols-outlined text-[#00288e] text-[18px]">badge</span>
                  <h4 className="text-[13px] font-bold text-[#1a1b22]">
                    Atur Nama Admin & Owner pada Template Laporan
                  </h4>
                </div>
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="text-[11px] text-[#757684] hover:text-[#00288e] underline cursor-pointer"
                  >
                    Reset Semula
                  </button>
                  <button
                    type="button"
                    onClick={() => setIsEditingSigners(false)}
                    className="px-3 py-1 bg-[#00288e] text-white rounded-lg text-[11px] font-bold hover:bg-[#1e40af] cursor-pointer"
                  >
                    Simpan & Tutup
                  </button>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-2.5">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-[#444653] uppercase tracking-wider block">
                    Nama Admin / Pembuat:
                  </label>
                  <input
                    type="text"
                    value={signerConfig.creatorName}
                    onChange={(e) => setSignerConfig((prev) => ({ ...prev, creatorName: e.target.value }))}
                    placeholder="Nama Admin"
                    className="w-full px-2.5 py-1.5 bg-white border border-[#c4c5d5] rounded-lg text-[12px] font-semibold text-[#1a1b22] focus:outline-none focus:border-[#00288e]"
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="text"
                    value={signerConfig.creatorTitle}
                    onChange={(e) => setSignerConfig((prev) => ({ ...prev, creatorTitle: e.target.value }))}
                    placeholder="Jabatan Admin"
                    className="w-full px-2.5 py-1.5 bg-white border border-[#c4c5d5] rounded-lg text-[12px] text-[#1a1b22] focus:outline-none focus:border-[#00288e]"
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="text"
                    value={signerConfig.approverName}
                    onChange={(e) => setSignerConfig((prev) => ({ ...prev, approverName: e.target.value }))}
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="text"
                    value={signerConfig.approverTitle}
                    onChange={(e) => setSignerConfig((prev) => ({ ...prev, approverTitle: e.target.value }))}
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="text"
                    value={signerConfig.signatureCity}
                    onChange={(e) => setSignerConfig((prev) => ({ ...prev, signatureCity: e.target.value }))}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-[#8e9099]/15 flex justify-center">
          ...
          >
            ...
            >
              ...
              <div className="pt-6 border-t border-[#c4c5d5]/40 mt-6 text-[12px] text-[#1a1b22]">
                <div className="flex justify-end mb-3">
                  <div className="text-right">
                    <span className="text-[#444653] font-medium text-[11px]">
                      {signerConfig.signatureCity || 'Tangerang Selatan'}, {currentDateFormatted}
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-12 text-center">
                  <div>
                    ...
                    <div className="h-16 flex items-end justify-center pb-1">
                      <span className="font-bold underline text-[#1a1b22] text-[13px]">
                        {signerConfig.creatorName || currentUser?.name || 'Administrator'}
                      </span>
                    </div>
                    <p className="text-[11px] text-[#757684]">
                      {signerConfig.creatorTitle || 'Staff Inventaris & Keuangan'}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="h-16 flex items-end justify-center pb-1">
                      <span className="font-bold underline text-[#1a1b22] text-[13px]">
                        {signerConfig.approverName || 'Pemilik / Owner ADDA RASA'}
                      </span>
                    </div>
                    <p className="text-[11px] text-[#757684]">
                      {signerConfig.approverTitle || 'Owner / Direktur Utama'}
                    </p>
                  </div>
                </div>

                {/* Quick Edit button right inside preview canvas (hidden on print) */}
                <div className="mt-4 text-center no-print">
                  <button
                    type="button"
                    onClick={() => setIsEditingSigners(true)}
                    className="inline-flex items-center gap-1 px-3 py-1 bg-[#f4f2fc] hover:bg-[#eeedf7] text-[#00288e] rounded-lg text-[11px] font-bold transition-all border border-[#00288e]/20 cursor-pointer"
                  >
                    <span className="material-symbols-outlined text-[14px]">edit</span>
                    <span>Ubah Nama Admin / Owner Laporan</span>
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="p-4 md:p-5 border-t border-[#c4c5d5]/30 bg-[#fbf8ff] flex flex-col sm:flex-row justify-between items-center gap-3">
          <div className="text-[12px] text-[#757684] flex items-center gap-2">
            {isGeneratingPdf ? (
              ...
              <div className="flex items-center gap-2 text-[#00288e] font-semibold animate-pulse">
                <span className="material-symbols-outlined text-[18px] animate-spin">print</span>
