...
export const App: React.FC = () => {
  ...
  const [transactions, setTransactions] = useState<Transaction[]>(() => {
    try {
      const purgeKey = 'addarasa_transactions_cleared_all_in_out_v5';
      if (!localStorage.getItem(purgeKey)) {
        localStorage.removeItem('addarasa_transactions');
    ...
    if (webAppUrl && webAppUrl.startsWith('https://script.google.com/')) {
      fetchAllFromAppsScript(webAppUrl)
        .then((remote) => {
          ...
          const isPurged = localStorage.getItem('addarasa_transactions_cleared_all_in_out_v5');
          if (!isPurged && remote.transactions && remote.transactions.length > 0) {
  ...
  const handleClearStockAndTransactions = () => {
    ...
    try {
      localStorage.removeItem('addarasa_transactions');
      localStorage.setItem('addarasa_transactions_cleared_all_in_out_v5', 'true');
    } catch {
      // ignore
    }
  };
  ...
  const handleDeleteTransaction = (transactionId: string) => {
    setTransactions((prev) => {
      ...
      try {
        localStorage.setItem('addarasa_transactions', JSON.stringify(updated));
      } catch {
        // ignore
      }
      return updated;
