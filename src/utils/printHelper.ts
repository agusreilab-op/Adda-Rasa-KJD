...
/**
 * Helper to collect all active CSS styles and stylesheets from parent document
 */
export const getActiveStylesheetsHtml = (): string => {
  ...
  try {
    ...
    console.warn('Could not collect stylesheets:', e);
    return '';
  }
};
...
export const buildPrintableDocumentHtml = (
  ...
  const parentStyles = getActiveStylesheetsHtml();
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Cinzel:wght@600;700;800&family=Inter:wght@400;500;600;700;800;900&family=JetBrains+Mono:wght@400;500;700&display=swap" rel="stylesheet">
...
<body>
  <div class="action-toolbar">
    <div>
      <h2>${documentTitle}</h2>
      <span style="font-size: 11px; opacity: 0.85;">ADDA RASA KJD &bull; Dokumen Resmi Siap Cetak</span>
    </div>
    <div class="btn-group">
      <button class="btn btn-print" onclick="window.print()">
        <span>🖨️ Cetak ke Printer</span>
      </button>
...
export const printElement = (
  element: HTMLElement | null,
  documentTitle: string = 'Laporan ADDA RASA KJD',
  ...
  onStatusChange?.('Menyiapkan printer...');

  // Strategy 1: Hidden Iframe with desktop dimensions so CSS flex/grid and tables render accurately
  try {
    ...
    // Use desktop viewport size instead of 0x0 so styles/fonts/tables render properly before printing
    ...
    if (doc) {
      const fullHtml = buildPrintableDocumentHtml(element.innerHTML, documentTitle, false);
      doc.open();
      doc.write(fullHtml);
      doc.close();
      ...
      setTimeout(() => {
        try {
          ...
          onStatusChange?.('Dialog printer sistem terbuka.');
        } catch (iframeErr) {
          console.warn('Iframe print failed due to sandbox constraints, falling back:', iframeErr);
          // Try opening in new tab which bypasses iframe sandbox completely
          openDocumentInNewTab(element.innerHTML, documentTitle, true);
        } finally {
          setTimeout(() => {
    ...
    console.warn('Iframe print initialization failed, trying direct new tab:', err);
  }

  // Strategy 2: Open in new tab with auto-print
  try {
    openDocumentInNewTab(element.innerHTML, documentTitle, true);
    return true;
  } catch (err) {
    console.warn('New tab strategy failed, attempting direct print mount:', err);
...
export const openDocumentInNewTab = (
  ...
  const fullHtml = buildPrintableDocumentHtml(contentHtml, documentTitle, autoPrint);
